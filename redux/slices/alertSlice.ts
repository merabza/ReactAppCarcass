...
import type { Err } from "../types/errorTypes";

interface IAlertState {
    alert: { [key: string]: Err[] };
}

export const EAlertKind = {
    ApiLoad: "ApiLoad",
...
export type EAlertKind = (typeof EAlertKind)[keyof typeof EAlertKind];
        ...
        setAlertApiMutationError: (state, action: PayloadAction<Err[]>) => {
            ...
            if (!(EAlertKind[alertKind] in alert)) {
                ...
                const newErrors = errors.filter(
                    (e: { errorCode: string }) =>
                        !existedErrors.find((ee) => ee.errorCode == e.errorCode)
                );
                state.alert[EAlertKind[alertKind]] =
